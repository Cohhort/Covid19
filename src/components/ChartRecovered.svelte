...
import { colorPalet, getArrayColor, fetchDatas, getCountries, getDates, getRecovered } from './../helpers/datas.js'
...
onMount(async () => {
  ...
  datas = countries.map((country,i)=>{
    return {
      country: country,
      datas: getRecovered(tab.filter(row=>row[0]==country))
    }
  })
  ...
  var ctx = document.getElementById("chartRecovered").getContext("2d");
