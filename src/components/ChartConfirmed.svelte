...
import { colorPalet, getArrayColor, fetchDatas, getCountries, getDates, getConfirmed } from './../helpers/datas.js'
...
onMount(async () => {
  ...
  datas = countries.map((country,i)=>{
    return {
      country: country,
      datas: getConfirmed(tab.filter(row=>row[0]==country))
    }
  })
  ...
  var ctx = document.getElementById("chartConfirmed").getContext("2d");
    ...
    <div class="max-w-screen-xl px-4 mx-auto my-10 sm:mt-12 sm:px-6 md:mt-16 lg:mt-20 lg:px-8 xl:mt-28">
      <div class="sm:text-center lg:text-left">
        <h2 class="mb-24 text-4xl font-bold leading-10 tracking-tight text-gray-800 font-display sm:text-5xl sm:leading-none md:text-52xl">
          Evolution of <span class="text-coorange">Confirmed Cases</span> over the world
        </h2>
